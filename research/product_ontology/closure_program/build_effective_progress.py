...
def main():
    ...
    b07_current=J('research/domain_atlas/industries/canonical-reference-current-method-summary.json') if exists('research/domain_atlas/industries/canonical-reference-current-method-summary.json') else {'candidate_records':0,'candidate_occurrences':0}
    ...
    ctx_route=J('research/domain_atlas/context_map/effective-gap-routing-summary.json') if exists('research/domain_atlas/context_map/effective-gap-routing-summary.json') else J('research/domain_atlas/context_map/manifest.json')
    ...
    statuses['B06_LIBRARY_CONTRACT_AUTHORITY']={'state':'STRUCTURAL_SCOPE_DONE_AUTHORITY_BLOCKED','remaining':{'exact_semantic_scopes':b06['semantic_contract_scope_count'],'ratified':b06['ratified_contract_scope_count']},'machine_addressable':False}
    effective_b07=max(0,b07['remaining_without_manual_or_machine_exact_candidate']-b07_current.get('candidate_records',0))
    statuses['B07_INDUSTRY_CANONICAL_REFERENCE_CLOSURE']={'state':'MACHINE_ACTIVE_SEMANTIC_REVIEW','remaining':{'unclassified_reference_rows':effective_b07,'machine_exact_candidates':b07['machine_exact_alias_candidate_records']+b07_current.get('candidate_records',0)},'machine_addressable':True}
    statuses['B08_SOURCE_SYSTEM_AND_OCCURRENCE_CLOSURE']={'state':'MACHINE_ACTIVE','remaining':{'retained_occurrences':occ['retained_occurrence_count'],'source_classes_with_occurrence':occ['source_class_count_with_retained_occurrence'],'source_classes_without_retained_occurrence':171-occ['source_class_count_with_retained_occurrence'],'production_qualified_occurrences':occ['production_qualified_occurrence_count']},'machine_addressable':True}
    statuses['B09_DATA_SHAPE_OPERATION_TOTALITY']={'state':'MACHINE_ACTIVE','remaining':{'historical_gaps':dsg['historical_gap_count'],'end_to_end_closed':dsg['end_to_end_closed_gap_count'],'current_dispositions_explicit':True},'machine_addressable':True}
    statuses['B10_IMPLEMENTATION_IDENTITY_AND_BUILD']={'state':'MACHINE_ACTIVE_FACTORED','remaining':{'exact_contract_scopes':harness['semantic_contract_scope_count'],'harness_campaigns':harness['harness_campaign_count'],'qualification_verdicts':harness['qualification_verdicts_created']},'machine_addressable':True}
    ...
    for b in master:
        ...
        rows.append({'batch_id':b['batch_id'],'depends_on':b.get('depends_on',[]),'effective_state':s['state'],'remaining':s['remaining'],'machine_addressable_now':s['machine_addressable'],'exit_condition':b['exit_condition'],'completion_claim':False})
    (HERE/'effective-batch-progress.jsonl').write_text(''.join(json.dumps(r,sort_keys=True)+'\n' for r in rows),encoding='utf-8')
    program_obligations=program['b16_coverage_coordinates']+program['b17_seeded_solution_challenges']+program['b18_human_effect_obligations']+program['b19_system_fault_obligations']+program['b20_invalidation_obligations']
    summary={'report_id':'effective_batched_closure_progress','as_of':'2026-08-27','batch_count':len(rows),'structurally_done_or_done_monitor_count':sum('DONE' in r['effective_state'] for r in rows),'machine_addressable_batch_count':sum(r['machine_addressable_now'] for r in rows),'authority_or_external_evidence_blocked_batch_count':sum(('AUTHORITY_BLOCKED' in r['effective_state'] or 'EVIDENCE_BLOCKED' in r['effective_state'] or 'INDEPENDENCE_BLOCKED' in r['effective_state']) for r in rows),'effective_qualification_vacancies':q['effective_evidence_vacancy_count'],'program_level_obligation_count':program_obligations,'program_level_acceptance_results':program['program_gate_acceptance_results'],'largest_machine_queue':{'batch_id':'B07_INDUSTRY_CANONICAL_REFERENCE_CLOSURE','count':effective_b07},'retained_source_occurrences':occ['retained_occurrence_count'],'exact_contract_scopes':harness['semantic_contract_scope_count'],'qualification_harness_campaigns':harness['harness_campaign_count'],'completion_claim':False}
