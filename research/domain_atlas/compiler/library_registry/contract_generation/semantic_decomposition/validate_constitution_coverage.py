...
def main()->int:
    ...
    assert coverage["axis_count"]==16 and coverage["family_count"]==23 and coverage["family_axis_matrix_count"]==368
    assert len({x["axis"] for x in queue})==16 and len({x["family_id"] for x in queue})==23
    assert len({(x["axis"],x["family_id"]) for x in queue})==368
    ...
    print("PASS semantic constitution coverage: all 16 axes map exactly once to five candidate constitutions and 368 family inheritance matrices; owner ratification remains open");return 0
