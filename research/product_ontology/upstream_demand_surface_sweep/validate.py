...
assert len(boundaries)>=20 and all(set(r['source_refs'])<=sources for r in boundaries)
assert len(contracts)>=120 and len({r['contract_hypothesis_id'] for r in contracts})==len(contracts)
assert len(packs)>=10
