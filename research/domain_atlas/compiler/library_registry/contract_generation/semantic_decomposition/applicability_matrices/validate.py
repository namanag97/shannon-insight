...
def main()->int:
    ...
    assert len({x["preclassification_id"] for x in rows})==len(rows)
    assert len({(x["library_ref"],x["axis"]) for x in rows})==len(rows)
    assert len({(x["family_id"],x["axis"]) for x in matrices})==len(matrices)
    ...
    assert len(member_cluster_refs)==len(set(member_cluster_refs))==len(rows) and set(member_cluster_refs)==rowrefs
    ...
    assert len(b["waves"])==5 and sum(x["member_rows"] for x in b["waves"])==len(rows)
    print(f"PASS semantic applicability matrices: {len(rows)} cells covered exactly once by {len(matrices)} family matrices and {len(clusters)} conservative candidate clusters; no implicit applicability or closure")
    return 0
