...
def main() -> int:
    ...
    assert len(members) == len({row["library_ref"] for row in members}) == 674
    ...
    assert summary["routes_with_lexical_discovery_projection"] + summary["routes_with_no_member_temporal_bearer_evidence"] == 674
    ...
    print(
        f"PASS time coordinate ontology: 674 exact members partition losslessly into "
