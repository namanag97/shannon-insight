#!/usr/bin/env python3
"""Audit and receipt every upstream family corpus without granting semantic authority."""
...
def build()->dict[str,Any]:
    packages=load_jsonl(PACKAGES);receipts={x["family_id"]:x for x in load_jsonl(RECEIPTS)};assert len(packages)==len(receipts)
    assert {x["family_id"] for x in packages}==set(receipts)
    audits=[]
