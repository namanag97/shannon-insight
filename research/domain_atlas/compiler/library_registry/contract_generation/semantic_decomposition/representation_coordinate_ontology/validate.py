...
def main() -> int:
    ...
    assert len(built["dockets"]) == len(built["extensions"]) == 23
    assert len(built["members"]) == len({row["library_ref"] for row in built["members"]}) == 674
    assert sum(row["library_count"] for row in built["clusters"]) == 674
    assert summary["routes_with_lexical_representation_projection"] == 175
    assert summary["routes_with_no_representation_projection"] == 499
    ...
    print(
        "PASS representation coordinate ontology: 674 exact members partition losslessly into "
