...
def main():
    ...
    for a in artifacts:
        ...
        values=[aid,a.get('name','')]
        # Exact current method identities may expose simple library names such as
        # "Forecast Estimators". We do not strip domain-specific words beyond namespace/punctuation.
        for v in values:
            f=norm(v)
    ...
    for q in queue:
        ...
        rows.append({
            'record_kind':'canonical_reference_current_method_identity_candidate',
            'candidate_id':'currentmethod.'+hashlib.sha256(q['queue_id'].encode()).hexdigest()[:16],
            'edition':1,'queue_id':q['queue_id'],'reference_domain':'analytical_practice',
            'raw_ref':q['raw_ref'],'normalized_form':form,'candidate_target_ref':target,
            'candidate_target_kind':a['kind'],'candidate_target_name':a.get('name'),
            'candidate_semantic_owner_ref':a.get('semantic_owner_ref'),
            'occurrence_count':q['occurrence_count'],'origin_packs':q['origin_packs'],
            'relation':'exact_current_adjudicated_method_identity_candidate',
            'status':'PROPOSED_NOT_RATIFIED','completion_claim':False,
            'decision_basis':['unique equality after NFKC/case/punctuation/namespace normalization','target is a current adjudicated semantic_contract or library_contract, not a product/capability/provider'],
            'prohibited_inferences':['semantic broadening or narrowing','mapping a lifecycle/workbench/product capability as a method','compiler substitution before authority','treating a library implementation as semantic ratification']
        })
