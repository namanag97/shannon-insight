...
class TestDetectRenames:
    ...
    @pytest.mark.skipif(
        ...
        and not os.path.exists("/usr/local/bin/git")
        and not os.path.exists("/opt/homebrew/bin/git"),
    ...
    def test_detect_renames_in_temp_repo(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            subprocess.run(
                ["git", "-C", tmpdir, "config", "user.name", "Test"], capture_output=True
            )
            ...
            # Rename file
            subprocess.run(
                ["git", "-C", tmpdir, "mv", "old_name.py", "new_name.py"], capture_output=True
            )
