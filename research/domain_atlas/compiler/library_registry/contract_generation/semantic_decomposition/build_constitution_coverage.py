...
def build()->dict[str,Any]:
    ...
    coverage={"record_kind":"semantic_constitution_coverage","coverage_id":"coverage.semantic-axis.constitutions.v1","edition":1,"as_of":AS_OF,"status":"ALL_GLOBAL_CONSTITUTIONS_CANDIDATE_COMPLETE_OWNER_RATIFICATION_OPEN","completion_claim":False,"axis_count":len(axis_to_module),"family_count":len(family_ids),"family_axis_matrix_count":len(inheritance),"phases":phases,"axis_bindings":[{"axis":axis,**binding} for axis,binding in sorted(axis_to_module.items())],"execution_rule":f"Ratify {len(phases)} global phase constitutions once; then process {len(family_ids)} family exception matrices per axis. Never copy full constitutions into every exact library candidate and never infer applicability from lexical matches.","canonical_gap_effect":0}
