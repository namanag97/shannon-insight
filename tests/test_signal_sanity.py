...
class TestFileSignals:
    ...
    def test_lines_signal(self, latest_snapshot_signals):
        """lines: Should be non-negative integer (can be 0 for empty files)"""
        ...
        for file_path, signals in file_signals.items():
            ...
            assert signals["lines"] < 100000, (
                f"{file_path} has unrealistic lines={signals['lines']}"
            )
    ...
    def test_risk_score_signal(self, latest_snapshot_signals):
        ...
        for file_path, signals in file_signals.items():
            if "risk_score" in signals:  # Composite, may not always be present
                assert 0 <= signals["risk_score"] <= 1, (
                    f"{file_path} risk_score={signals['risk_score']} out of range"
                )
        ...
        for file_path, signals in file_signals.items():
            if "blast_radius_size" in signals:
                ...
                assert signals["blast_radius_size"] <= len(file_signals), (
                    "Blast radius can't exceed file count"
                )
    ...
    def test_churn_signals(self, latest_snapshot_signals):
        ...
        high_cv_files = [
            (path, s) for path, s in file_signals.items() if s.get("churn_cv", 0) > 1.0
        ]
    ...
    def test_bus_factor_signal(self, latest_snapshot_signals):
        ...
        for file_path, signals in file_signals.items():
            if "bus_factor" in signals:
                ...
                assert signals["bus_factor"] >= 1.0, (
                    f"{file_path} bus_factor={signals['bus_factor']} < 1.0"
                )
    ...
    def test_concept_signals(self, latest_snapshot_signals):
        ...
        for path, signals in file_signals.items():
            if signals.get("concept_count") == 1:
                ...
                assert signals.get("concept_entropy", 0) == 0, (
                    f"{path} has 1 concept but entropy={signals.get('concept_entropy')}"
                )
...
class TestSignalCorrelations:
    ...
    def test_high_complexity_correlates_with_high_risk(self, latest_snapshot_signals):
        ...
        sorted_by_complexity = sorted(
            file_signals.items(), key=lambda x: x[1].get("cognitive_load", 0), reverse=True
        )[:10]
    ...
    def test_high_pagerank_means_high_blast_radius(self, latest_snapshot_signals):
        ...
        sorted_by_pagerank = sorted(
            file_signals.items(), key=lambda x: x[1].get("pagerank", 0), reverse=True
        )[:10]

        # These should have above-average blast radius
        avg_blast = sum(s.get("blast_radius_size", 0) for _, s in file_signals.items()) / len(
            file_signals
        )

        high_pr_avg_blast = sum(s.get("blast_radius_size", 0) for _, s in sorted_by_pagerank) / len(
            sorted_by_pagerank
        )
