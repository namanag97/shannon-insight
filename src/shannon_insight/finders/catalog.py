...
FINDERS = [
    ...
    Finder(
        name="GOD_FILE",
        condition=lambda s: (
            s.lines > 500 and (s.percentiles.get("complexity", 0) > 0.90 or s.in_degree > 10)
        ),
        severity="HIGH",
    ...
    Finder(
        ...
        condition=lambda s: s.bus_factor < 2 and s.percentiles.get("pagerank", 0) > 0.75,
        severity="HIGH",
    ...
    Finder(
        name="VOLATILE_CORE",
        condition=lambda s: s.percentiles.get("pagerank", 0) > 0.75 and s.churn_cv > 0.8,
        severity="HIGH",
    ...
    Finder(
        ...
        condition=lambda s: s.percentiles.get("complexity", 0) > 0.80 and s.total_changes > 20,
        severity="MEDIUM",
