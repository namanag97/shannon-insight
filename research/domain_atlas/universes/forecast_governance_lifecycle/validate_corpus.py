...
def main():
 before={p.name:p.read_bytes() for p in R.glob("*.json*")}; out=subprocess.run([sys.executable,str(R/"build_corpus.py")],capture_output=True,text=True,check=True); after={p.name:p.read_bytes() for p in R.glob("*.json*")}; assert before==after,"generated drift"
 ...
 assert len(s)>=28 and len(c)==8 and len(d)>=190 and len(l)==8 and len(x)==12 and len(n)>=45 and len(p)==8
 ...
 print(out.stdout.strip()); print(f"VALIDATION PASS fgl: {sum(len(z['operation_refs']) for z in l)} operations; 5 typed open gaps; zero qualified implementations")
