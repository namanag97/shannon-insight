#!/usr/bin/env python3
"""Validate the search/information-retrieval semantic slice."""
import hashlib
import json

from build_search_information_retrieval_semantic_slice import (
...
def main() -> int:
    ...
    assert len(module_ids) == len(built["modules"]) == 48
    ...
    frontier = next(row for row in load_jsonl(HERE.parent / "analytical_formalism_frontier/formalism-frontier-clusters.jsonl") if row["cluster_id"] == "formalism.search_information_retrieval")
    assert set(frontier["declared_concrete_library_refs"]) == direct
    assert set(frontier["product_refs"]) == PRODUCTS
    assert len(direct) == 12 and len(NEIGHBORS) == 45 and len(LIBRARIES) == 57
    ...
    assert len({row["method_type_id"] for row in built["methods"]}) == len(built["methods"]) == 192
    ...
    assert len(bindings) == 57 and {row["library_ref"] for row in bindings} == set(LIBRARIES)
    ...
    assert len(vacancy_refs) == len(VACANCIES) == 28 and vacancy_refs.isdisjoint(LIBRARIES)
    ...
    expected_dispositions = {
        "finding.search.products.retain-separate.v1": "RETAIN_SEARCH_AND_METADATA_DISCOVERY_AS_INDEPENDENT_PRODUCTS",
        "finding.search.vector-method.v1": "KEEP_VECTOR_AND_ANN_AS_METHOD_CAPABILITIES_NOT_AI_PRODUCT",
        "finding.search.model-authority.v1": "IMPORT_RANKING_AND_EMBEDDING_MODEL_LIFECYCLE",
        "finding.search.discovery-authority.v1": "KEEP_ASSERTION_AUTHORITY_IN_METADATA_DISCOVERY_NOT_SEARCH_INDEX",
        "finding.search.query-homonym.v1": "SPLIT_QUERY_HOMONYM_INTO_EXACT_GRAPH_RANKED_BROWSE_AND_DISCOVERY_TYPES",
    }
    ...
    assert built["context"]["product_boundary_candidates"] == [
        {"product_ref": "product.metadata_discovery", "status": "RETAIN_BUT_NARROW_UNRATIFIED"},
        {"product_ref": "product.search_index_service", "status": "RETAIN_BUT_NARROW_UNRATIFIED"},
    ]
    assert built["context"]["candidate_new_products"] == []

    summary = built["summary"]
    assert summary["bound_libraries"] == 57 and summary["library_axis_decision_candidates"] == 912
    assert summary["candidate_new_products"] == 0 and summary["candidate_new_library_vacancies"] == 28
    ...
    print("PASS search/information-retrieval semantic slice: 48 modules, 192 methods, 26 experts and 15 innovations bind the exact 12-library two-product graph plus 45 justified neighbors, retain Search and Metadata Discovery separately, expose 28 library vacancies and preserve 912 unresolved axis decisions")
    return 0
